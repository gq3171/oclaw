...
impl PluginRegistry {
    ...
    pub async fn initialize(&self, plugin_id: &str) -> PluginResult<()> {
        ...
        if let Some(wrapper) = plugins.get_mut(plugin_id) {
            wrapper.initialize().await.map_err(PluginError::InitError)?;
    ...
    pub async fn start(&self, plugin_id: &str) -> PluginResult<()> {
        ...
        if let Some(wrapper) = plugins.get_mut(plugin_id) {
            wrapper.start().await.map_err(PluginError::ExecutionError)?;
    ...
    pub async fn stop(&self, plugin_id: &str) -> PluginResult<()> {
        ...
        if let Some(wrapper) = plugins.get_mut(plugin_id) {
            wrapper.stop().await.map_err(PluginError::ExecutionError)?;
...
impl HookPipeline {
        ...
        >,
    {
        ...
        let mut sorted: Vec<&PluginWrapper> =
            plugins.values().filter(|p| p.config().enabled).collect();
    ...
    pub async fn before_request(&self, request: &str) -> Result<String, String> {
        self.run_transform(request, |p, v| {
            Box::pin(async move { p.inner().hook_before_request(&v).await })
        })
        .await
    }

    pub async fn after_response(&self, response: &str) -> Result<String, String> {
        self.run_transform(response, |p, v| {
            Box::pin(async move { p.inner().hook_after_response(&v).await })
        })
        .await
    }
    ...
    pub async fn before_message(&self, message: &str) -> Result<String, String> {
        self.run_transform(message, |p, v| {
            Box::pin(async move { p.inner().hook_before_message(&v).await })
        })
        .await
    }
    ...
    pub async fn after_message(&self, message: &str) -> Result<String, String> {
        self.run_transform(message, |p, v| {
            Box::pin(async move { p.inner().hook_after_message(&v).await })
        })
        .await
    }
    ...
    pub async fn content_filter(&self, content: &str) -> Result<String, String> {
        self.run_transform(content, |p, v| {
            Box::pin(async move { p.inner().hook_content_filter(&v).await })
        })
        .await
    }
    ...
    pub async fn gateway_startup(&self) {
        self.run_notify(|p| Box::pin(async move { p.inner().hook_gateway_startup().await }))
            .await;
    }

    pub async fn gateway_shutdown(&self) {
        self.run_notify(|p| Box::pin(async move { p.inner().hook_gateway_shutdown().await }))
            .await;
    }
    ...
    pub async fn before_compaction(&self, messages: &str) -> Result<String, String> {
        self.run_transform(messages, |p, v| {
            Box::pin(async move { p.inner().hook_before_compaction(&v).await })
        })
        .await
    }

    pub async fn after_compaction(&self, summary: &str) -> Result<String, String> {
        self.run_transform(summary, |p, v| {
            Box::pin(async move { p.inner().hook_after_compaction(&v).await })
        })
        .await
    }
    ...
    pub async fn subagent_delivery_target(
        &self,
        child_session_key: &str,
        payload: &str,
    ) -> Result<String, String> {
        let child_key = child_session_key.to_string();
        self.run_transform(payload, move |p, v| {
            let key = child_key.clone();
            Box::pin(async move { p.inner().hook_subagent_delivery_target(&key, &v).await })
        })
        .await
    }
