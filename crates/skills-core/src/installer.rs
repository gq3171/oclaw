...
async fn run_cmd(args: &[&str], timeout: std::time::Duration) -> InstallResult {
    ...
    let Ok(child) = child else {
        return InstallResult {
            ok: false,
            message: format!("Failed to spawn {cmd}"),
            stdout: String::new(),
            stderr: String::new(),
            code: None,
        };
...
async fn install_brew(spec: &InstallSpec, timeout: std::time::Duration) -> InstallResult {
    let Some(formula) = &spec.formula else {
        return InstallResult {
            ok: false,
            message: "brew spec missing formula".into(),
            stdout: String::new(),
            stderr: String::new(),
            code: None,
        };
...
async fn install_node(spec: &InstallSpec, timeout: std::time::Duration) -> InstallResult {
    let Some(package) = &spec.package else {
        return InstallResult {
            ok: false,
            message: "node spec missing package".into(),
            stdout: String::new(),
            stderr: String::new(),
            code: None,
        };
...
async fn install_go(spec: &InstallSpec, timeout: std::time::Duration) -> InstallResult {
    let Some(module) = &spec.module else {
        return InstallResult {
            ok: false,
            message: "go spec missing module".into(),
            stdout: String::new(),
            stderr: String::new(),
            code: None,
        };
...
async fn install_uv(spec: &InstallSpec, timeout: std::time::Duration) -> InstallResult {
    let Some(package) = &spec.package else {
        return InstallResult {
            ok: false,
            message: "uv spec missing package".into(),
            stdout: String::new(),
            stderr: String::new(),
            code: None,
        };
...
async fn install_download(spec: &InstallSpec, timeout: std::time::Duration) -> InstallResult {
    let Some(url) = &spec.url else {
        return InstallResult {
            ok: false,
            message: "download spec missing url".into(),
            stdout: String::new(),
            stderr: String::new(),
            code: None,
        };
    ...
    if archive.ends_with(".zip") {
        ...
        // tar.gz / tar.bz2
        let tar_flag = if archive.ends_with(".bz2") {
            "-xjf"
        } else {
            "-xzf"
        };
        run_cmd(
            &[
                "curl", "-fsSL", url, "|", "tar", tar_flag, "-", "-C", target,
            ],
            timeout,
        )
        .await
        .pipe_fallback(url, tar_flag, target, timeout)
        .await
    }
}
