...
use oclaw_protocol::frames::{EventFrame, GatewayFrame, RequestFrame, ResponseFrame};
...
impl MessageHandler {
    pub fn new_request(method: &str, params: Option<serde_json::Value>) -> RequestFrame {
        RequestFrame {
            frame_type: oclaw_protocol::frames::RequestFrameType::Req,
    ...
    pub fn new_response(
        ...
        error: Option<oclaw_protocol::frames::ErrorDetails>,
    ) -> ResponseFrame {
        ResponseFrame {
            frame_type: oclaw_protocol::frames::ResponseFrameType::Res,
            id: id.to_string(),
            ok,
            payload,
            error,
    ...
    pub fn new_event(event: &str, payload: Option<serde_json::Value>) -> EventFrame {
        EventFrame {
            frame_type: oclaw_protocol::frames::EventFrameType::Event,
...
impl SessionManager {
    pub fn new() -> Self {
        let db = rusqlite::Connection::open_in_memory().expect("Failed to open in-memory SQLite");
        Self::init_db(&db);
        Self {
    ...
    fn init_db(db: &rusqlite::Connection) {
        ...
        let migrations: &[&str] = &[
            ...
            // v3: session metadata key-value storage
            ...
            CREATE INDEX IF NOT EXISTS idx_session_metadata_key ON session_metadata(session_key);",
        ];
        ...
        for (i, sql) in migrations.iter().enumerate() {
            ...
            if ver > current {
                db.execute_batch(sql)
                    .unwrap_or_else(|e| panic!("Migration v{} failed: {}", ver, e));
                db.execute(
    ...
    pub fn set_session_metadata_field(
        ...
        let db = self
            ...
            .map_err(|e| format!("DB lock poisoned: {}", e))?;
        db.execute(
            "INSERT OR REPLACE INTO session_metadata (session_key, k, v) VALUES (?1, ?2, ?3)",
    ...
    pub fn set_session_metadata_fields(
        &self,
        key: &str,
