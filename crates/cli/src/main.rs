...
fn build_system_prompt(
    ...
    let mut prompt_exec =
        oclaw_gateway_core::http::agent_bridge::ToolRegistryExecutor::new(registry.clone())
            .with_llm_provider(provider.clone());
    if let Ok(cwd) = std::env::current_dir() {
        prompt_exec = prompt_exec.with_workspace_root(cwd);
    }
    oclaw_gateway_core::http::agent_bridge::build_default_agent_system_prompt(&prompt_exec)
}
...
async fn run_agent(
    ...
    let prompt = build_system_prompt(registry, provider);
